...
def build_model(num_classes):
    """Build EfficientNetB0 model with standardized architecture."""
...
def save_training_metrics(history, model_name="EfficientNetB0"):
...
def plot_training_history(history):
    ...
    plt.tight_layout()
    os.makedirs(CONFIG['model_export_path'], exist_ok=True)
    plt.savefig(os.path.join(CONFIG['model_export_path'], 'training_history_efficientnet_b0.png'))
    plt.close()

def convert_to_tflite(model):
    """Convert Keras model to TFLite format with robust error handling."""
    import shutil
    temp_model_path = os.path.join(CONFIG['model_export_path'], 'temp_saved_model_efficientnet')
    
    try:
        print("\n🔄 Converting EfficientNetB0 to TFLite...")
        ...
        # Save TFLite model
        tflite_path = os.path.join(CONFIG['model_export_path'], 'model_efficientnet_b0.tflite')
...
def evaluate_model(model, test_data):
    ...
    os.makedirs(CONFIG['model_export_path'], exist_ok=True)
    with open(os.path.join(CONFIG['model_export_path'], 'test_metrics_efficientnet_b0.json'), 'w') as f:
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(CONFIG['model_export_path'], 'confusion_matrix_efficientnet_b0.png'))
    plt.close()
...
def train_model():
    ...
    callbacks_phase1 = [
        ModelCheckpoint(
            os.path.join(CONFIG['model_export_path'], 'best_model_efficientnet_b0_phase1.keras'),
            monitor='val_loss',
            save_best_only=True,
            mode='min',
            verbose=1
        ...
        TensorBoard(log_dir=os.path.join(CONFIG['model_export_path'], 'logs_efficientnet_b0'), histogram_freq=1)
    ]
    ...
    callbacks_phase2 = [
        ModelCheckpoint(
            os.path.join(CONFIG['model_export_path'], 'best_model_efficientnet_b0.keras'),
    ...
    save_training_metrics(combined_history, "EfficientNetB0")
    ...
    # Save final model
    model.save(os.path.join(CONFIG['model_export_path'], 'final_model_efficientnet_b0.keras'))
    
    return model, history2

def main():
    print("\nStarting model training (EfficientNetB0)...")
    try:
